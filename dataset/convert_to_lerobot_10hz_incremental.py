...
def discover_project_root() -> Path:
    ...
    for candidate in (here.parents[2], *here.parents):
        if (candidate / "artifacts" / "Delta_Weights" / "config.json").is_file() and (
            candidate / "dataset"
        ).is_dir():
            return candidate
    raise ConverterError(
        "cannot locate project root containing artifacts/Delta_Weights"
    )
...
def validate_bundle_contract(project_root: Path) -> dict[str, Any]:
    bundle = project_root / "artifacts" / "Delta_Weights"
...
def _prepare_output(output: Path, source: Path, project_root: Path, overwrite: bool) -> None:
    ...
    immutable_model = (project_root / "artifacts" / "Delta_Weights").resolve()
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        / "artifacts/zarr/replay_buffer_yawfree.zarr",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
