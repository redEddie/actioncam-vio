...
MP4 ─┬─> IMU 추출        gopro_vio.extract (GPMF) / gopro_vio.insta360 (Insta360)
     ...
     └─> UMI 그리퍼      gopro_vio.aruco_detect / world_align / gripper_width
                        (태그 월드 앵커 + 그리퍼 너비 — docs/umi_gripper_pipeline.md)
```
...
- [ ] UMI 그리퍼: T_cam_tcp 캘리브·너비 오프셋 튜닝·에피소드 출력 (docs/umi_gripper_pipeline.md)

## 문서

- [UMI 그리퍼 파이프라인 end-to-end 검증](docs/umi_gripper_pipeline.md) — 그리퍼가
  화면을 가려도 궤적 생성이 되는지(된다), 태그 월드 앵커·너비 신호·FOV 크롭 실험
