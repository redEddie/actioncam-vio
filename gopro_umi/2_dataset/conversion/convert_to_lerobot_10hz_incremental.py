...
Each output row stores a six-dimensional state and a six-dimensional action:

    state  = [X, Y, Z, Roll, Pitch, Gripper]
    action = S[t + 1] - S[t]

The final row of each episode carries an explicit zero terminal action.  The
dataset therefore remains row-wise ``[6]``; SmolVLA constructs its ``[15, 6]``
future action window from dataset timestamps and ``chunk_size=15``.
"""
...
def discover_project_root() -> Path:
    ...
    for candidate in (here.parents[2], *here.parents):
        if (candidate / "Delta_Weights" / "config.json").is_file() and (
            candidate / "2_dataset"
        ).is_dir():
            return candidate
    raise ConverterError("cannot locate project root containing 2_dataset and Delta_Weights")
...
def validate_bundle_contract(project_root: Path) -> dict[str, Any]:
    config = json.loads((project_root / "Delta_Weights" / "config.json").read_text())
    train_config = json.loads((project_root / "Delta_Weights" / "train_config.json").read_text())
    preprocessor = json.loads(
        (project_root / "Delta_Weights" / "policy_preprocessor.json").read_text()
    )
    postprocessor = json.loads(
        (project_root / "Delta_Weights" / "policy_postprocessor.json").read_text()
    )
...
def compute_incremental_actions(states: np.ndarray) -> np.ndarray:
    """Return step-to-step actions with a zero action at the episode terminal."""
...
def audit_source(
    ...
    for episode_id in ids:
        ...
        reconstructed = selected_states[:-1].astype(np.float64) + actions[:-1].astype(np.float64)
        reconstruction_errors.append(
            np.abs(reconstructed - selected_states[1:].astype(np.float64))
        )
        sampled_frames += len(selection.indices)
    ...
    return {
        ...
        "terminal_action_policy": "zero 6D action on final sampled state of each episode",
...
def write_selected_episodes(
    ...
    for episode_id in episode_ids:
        ...
        selected_states = states[start + selection.indices]
        actions = compute_incremental_actions(selected_states)
        for local_row, source_local_index in enumerate(selection.indices):
...
def _prepare_output(output: Path, source: Path, project_root: Path, overwrite: bool) -> None:
    ...
    immutable_model = (project_root / "Delta_Weights").resolve()
    historical_archive = (project_root / "archive").resolve()
    if (
        target == project
...
def _create_lerobot_dataset(
    ...
    vendored_source = project_root / "third_party" / "lerobot" / "src"
...
def convert(args: argparse.Namespace) -> dict[str, Any]:
    ...
    dataset = _create_lerobot_dataset(project_root, output, args.repo_id)
    try:
    ...
    metadata_path = output / "meta" / "conversion_provenance.json"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input-zarr",
        type=Path,
        default=project_root / "archive/legacy_dataset/zarr/replay_buffer_yawfree.zarr",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=project_root
        / "2_dataset/lerobot_dataset_10hz_chunk15_incremental_reconstructed_v1",
    )
    parser.add_argument(
        "--repo-id",
        default="gopro_umi/lerobot_dataset_10hz_chunk15_incremental_reconstructed_v1",
    )
    parser.add_argument(
        "--source-fps",
        type=float,
