...
def safe_connect(port):
    ...
    # Force P=48, I=0, D=32 for Trial A start
...
def run_trial(follower, arm_names, target, label):
    ...
    if "Present_Temperature" in logs:
        ...
        print(f"Temperature Start: {t_start:.1f} °C, End: {t_end:.1f} °C, Delta: {t_end - t_start:.1f} °C")
    ...
    return {
        ...
        "velocity": np.mean(logs["Present_Velocity"][-150:], axis=0) if "Present_Velocity" in logs else None,
...
def main():
    ...
    try:
        ...
        # Trial A (P=48)
        metrics_A = run_trial(follower, arm_names, fixed_start_enc, "A (P=48)")
        ...
        if np.any(max_disp > 5.0):
            print("LARGE SNAP/OSCILLATION DETECTED! ABORTING P=64 TRIAL.")
            follower.bus.sync_write("P_Coefficient", {n: 48 for n in arm_names})
            # Hold position
            obs_curr = follower.bus.sync_read("Present_Position")
            follower.bus.sync_write("Goal_Position", obs_curr)
            time.sleep(1.0)
            return
        ...
        # Trial B (P=64)
        metrics_B = run_trial(follower, arm_names, fixed_start_enc, "B (P=64)")
        
        print("\n--- COMPARISON ---")
        idx = arm_names.index("elbow_flex")
        e48 = abs(metrics_A["mean_err"][idx])
        e64 = abs(metrics_B["mean_err"][idx])
        imp = (e48 - e64) / e48 * 100.0 if e48 > 0 else 0
        print(f"Elbow Improvement (P48 -> P64): {imp:.1f}%")
        
        print("\nOther Joints P48 MAE:")
        print(metrics_A["mae_err"])
        print("Other Joints P64 MAE:")
        ...
        print("P48:", metrics_A["sign_changes"])
        print("P64:", metrics_B["sign_changes"])
        
        print("\nPosition STD:")
        print("P48:", metrics_A["pos_std"])
        print("P64:", metrics_B["pos_std"])
        
        # Sync Current at the very end
        print("\nSyncing Goal to Present at script end...")
        obs_end = follower.bus.sync_read("Present_Position")
        follower.bus.sync_write("Goal_Position", obs_end)
        time.sleep(0.5)
