#!/usr/bin/env python3
"""STEP8.28: Large Effective Correction Gain Test

Test whether increasing K_ext from 1.0 to 2.0 (with a fixed clamp of +/-2.0)
improves steady tracking by generating a larger effective command.
"""
...
def run_session(*, port: str, log_dir: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, k_ext in enumerate(K_EXT_VALUES):
            ...
            result = lineage.run_trial(
                follower, ik_solver, list(ARM_NAMES), k_ext, start, log_dir, f"step8_28_kext_{k_ext:.1f}.npz",
            ...
            if previous_start is not None:
                ...
                if fk_delta > START_FK_CONFOUND_MM or max_q_diff > START_JOINT_CONF0UND_DEG:
                    print(f"CONFOUND K_ext={k_ext:.1f}: start FK={fk_delta:.3f} mm, q={max_q_diff:.3f} deg ({max_q_name}); retaining raw result")
                    confounded = True
...
def print_report(results: dict[str, Any]) -> None:
    print("========================================")
    print("STEP 8.28 LARGE EFFECTIVE CORRECTION GAIN TEST")
    print("========================================")
    print("1. STEP8.28 FILES\nmain file:\nstep8_28_large_gain_test.py\ntest file:\ntest_step8_28.py\nother modified files:\nNONE")
    ...
    if sd:
        ...
        print(f"A actual start:\n{np.round(sd['A_q'], 3)}")
        print(f"B actual start:\n{np.round(sd['B_q'], 3)}")
    ...
    for cond, k_ext in [("A", "1.0"), ("B", "2.0")]:
        res = results.get(cond)
        m = res["metrics"] if res and "metrics" in res else None
        print(f"\n{5 if cond == 'A' else 6}. CONDITION {cond} — K_ext={k_ext}")
        if m:
            ...
            print(f"max |q_corr_unclipped|:\n{np.round(m.max_abs_q_corr_unclipped_deg, 3)}")
            print(f"max |q_corr|:\n{np.round(m.max_abs_q_corr_deg, 3)}")
            print(f"hold mean |q_corr|:\n{np.round(m.hold_mean_abs_q_corr_deg, 3)}")
            print(f"hold P95 |q_corr|:\n{np.round(m.hold_p95_abs_q_corr_deg, 3)}")
            print(f"clamp-hit count:\n{m.clamp_hit_count_per_joint}")
    ...
    if m_a and m_b:
        elbow_idx = 2
        elbow_a = m_a.max_abs_q_corr_deg[elbow_idx]
        elbow_b = m_b.max_abs_q_corr_deg[elbow_idx]
        ratio = elbow_b / elbow_a if elbow_a > 1e-6 else float('inf')
        increased = elbow_b > elbow_a * 1.1 # At least 10% increase to be considered 'increased'
        
        print(f"\n7. EFFECTIVE CORRECTION INCREASE\nelbow A max |q_corr|:\n{elbow_a:.3f}\nelbow B max |q_corr|:\n{elbow_b:.3f}\nB/A ratio:\n{ratio:.3f}\nactual correction increased:\n{'YES' if increased else 'NO'}")
        
        impr = m_b.tracking_ratio_percent - m_a.tracking_ratio_percent
        print(f"\n8. TRACKING CHANGE\nA tracking:\n{m_a.tracking_ratio_percent:.1f} %\nB tracking:\n{m_b.tracking_ratio_percent:.1f} %\nB-A:\n{impr:.1f} percentage points")
        print(f">=10 pp:\n{'YES' if impr >= 10 else 'NO'}")
        print(f">=85%:\n{'YES' if m_b.tracking_ratio_percent >= 85 else 'NO'}")
        print(f">=90%:\n{'YES' if m_b.tracking_ratio_percent >= 90 else 'NO'}")
        
        ret_a = m_a.return_norm_mm
        ret_b = m_b.return_norm_mm
        print(f"\n9. RETURN\nA:\n{ret_a:.3f} mm\nB:\n{ret_b:.3f} mm\nB <=2.0 mm:\n{'YES' if ret_b <= 2.0 else 'NO'}\nB <=1.5 mm:\n{'YES' if ret_b <= 1.5 else 'NO'}")
    ...
    print("\n15. NEXT STEP\nexactly one recommendation:\nRun the hardware test to validate effective correction increase.")
    print("\n16. EXACT HOST COMMAND\npython3 /home/kimminje/Desktop/project/gopro_umi/4_deploy/step8_28_large_gain_test.py --execute --token STEP8_28_LARGE_GAIN_TEST")
    print("\n========================================")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.execute or args.token != EXECUTION_TOKEN:
        ...
        return 0
    if input("Type EXECUTE_STEP8_28 to continue: ").strip() != "EXECUTE_STEP8_28":
        raise RuntimeError("local confirmation did not match")
    log_dir = Path(__file__).resolve().parent / "logs" / "step8_28_large_gain_test"
