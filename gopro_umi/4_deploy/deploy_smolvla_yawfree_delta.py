...
def convert_delta_to_absolute(current_state: np.ndarray, predicted_delta_chunk: np.ndarray) -> np.ndarray:
    """
    AI 모델이 예측한 상대 변화량(Delta) 궤적을 절대 좌표 궤적으로 복원합니다.
    """
...
def run_dry_run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    raw_output = np.asarray(action_tensor.squeeze(0).detach().cpu(), dtype=np.float64)
    if raw_output.ndim == 1:
        raw_output = np.expand_dims(raw_output, axis=0)
    
    if raw_output.shape[-1] != 6 or not np.isfinite(raw_output).all():
        raise RuntimeError(f"invalid delta action: shape={raw_output.shape}, values={raw_output}")
        
    # ⭐️ [새로 추가한 로직] 델타값을 '절대 좌표'로 싹 바꿉니다.
    absolute_chunk = convert_delta_to_absolute(current_state, raw_output)
    
    # Dry-run 진단을 위해 첫 번째 스텝만 추출하여 사용
    action = absolute_chunk[0]
