...
def main():
    ...
    startup_log["initial_torque"] = tel0.get("Torque_Enable")
    startup_log["initial_P"] = tel0.get("P_Coefficient")
    startup_log["initial_I"] = tel0.get("I_Coefficient")
    startup_log["initial_D"] = tel0.get("D_Coefficient")
    startup_log["initial_Goal"] = tel0.get("Goal_Position")
    startup_log["initial_Present"] = tel0.get("Present_Position")
    
    follower.bus.sync_write("Goal_Position", {n: float(startup_log["initial_Present"][i]) for i,n in enumerate(arm_names)})
    time.sleep(0.05)
    
    goal_check = read_telemetry(follower, arm_names, ["Goal_Position"]).get("Goal_Position")
    diff = np.max(np.abs(goal_check - startup_log["initial_Present"]))
    startup_log["goal_sync_pass"] = (diff < 1.0)
    ...
    startup_log["pid_before_torque_pass"] = (np.all(pid_check.get("P_Coefficient")==64) and np.all(pid_check.get("I_Coefficient")==0) and np.all(pid_check.get("D_Coefficient")==32))
    ...
    for t_wait in t_waits:
        ...
        disp = np.abs(act - startup_log["initial_Present"])
        max_disp = np.maximum(max_disp, disp)
        transients[f"{int(t_wait*1000)}ms"] = disp
        
    startup_log["transients"] = transients
    startup_log["transient_max"] = max_disp
    
    if max_disp[2] > 2.0:
        print("ABORT: Elbow transient > 2.0deg")
        return
        
    pid_check2 = read_telemetry(follower, arm_names, ["P_Coefficient", "I_Coefficient", "D_Coefficient", "Torque_Enable"])
    startup_log["pid_after_torque"] = pid_check2
    startup_log["pid_after_torque_pass"] = (np.all(pid_check2.get("P_Coefficient")==64) and np.all(pid_check2.get("I_Coefficient")==0) and np.all(pid_check2.get("D_Coefficient")==32) and np.all(pid_check2.get("Torque_Enable")==1))
    
    if not startup_log["pid_after_torque_pass"]:
        print("ABORT: PID or Torque changed after torque enable")
        return
        
    print("Safe startup passed. Moving to fixed start...")
    
    urdf_path = DEPLOY_DIR / "URDF" / "so_arm_with_gopro_final.urdf"
    ik_solver = DLSInverseKinematicsV7(urdf_path=str(urdf_path))
    
    log_dir = DEPLOY_DIR / 'logs' / 'step8_16r_baseline'
    ...
    try:
        ...
        if np.max(np.abs(jump)) > 0.5:
            print("ABORT: FIRST COMMAND JUMP > 0.5 DEG")
            return
        ...
        print("Starting physical Cartesian test...")
        ...
        for step in range(steps):
            try:
                ...
                tick_logs["command_offset_from_nominal"].append(q_cmd_urdf_deg - np.rad2deg(q_nom))
                ...
                if tick_start - last_telemetry_t >= 1.0:
                    tel = read_telemetry(follower, arm_names, ["Present_Load", "Present_Temperature", "Torque_Enable"])
                    ...
                    if "Torque_Enable" in tel:
                        if np.any(tel["Torque_Enable"] == 0):
                            torque_interruption = True
                            print("TORQUE CONTENTION DETECTED")
                            break
        ...
        np.savez(log_dir / "step8_16r_full.npz", **tick_logs)
        np.savez(log_dir / "step8_16r_slow.npz", **slow_logs)
        np.savez(log_dir / "step8_16r_startup.npz", **startup_log)
        
        print("\n\n" + "="*40)
        print("STEP 8.16R CLEAN BASELINE REPLAY")
        print("================================")
        print("\n1. FRESH POWER CYCLE\nYES")
        print("\n2. follower.configure() CALLED?\nMUST BE NO")
        print("\n3. INITIAL TORQUE\n", startup_log["initial_torque"])
        print("\n4. INITIAL PID\nP:\n", startup_log["initial_P"], "\nI:\n", startup_log["initial_I"], "\nD:\n", startup_log["initial_D"])
        print("\n5. INITIAL GOAL\n", startup_log["initial_Goal"])
        print("\n6. INITIAL PRESENT\n", startup_log["initial_Present"])
        print("\n7. GOAL=Present SYNC\n", "PASS" if startup_log["goal_sync_pass"] else "FAIL")
        print("\n8. PID BEFORE TORQUE\nP:\n64\nI:\n0\nD:\n32\nread-back:\n", "PASS" if startup_log["pid_before_torque_pass"] else "FAIL")
        print("\n9. TORQUE-ON TRANSIENT\n50ms:\n", startup_log["transients"]["50ms"])
        print("100ms:\n", startup_log["transients"]["100ms"])
        print("200ms:\n", startup_log["transients"]["200ms"])
        print("500ms:\n", startup_log["transients"]["500ms"])
        print("1s:\n", startup_log["transients"]["1000ms"])
        print("2s:\n", startup_log["transients"]["2000ms"])
        print("max:\n", startup_log["transient_max"])
        print(f"elbow max:\n{startup_log['transient_max'][2]:.3f}")
        print("\n10. PID AFTER TORQUE ON\nP:\n", startup_log["pid_after_torque"].get("P_Coefficient"))
        print("I:\n", startup_log["pid_after_torque"].get("I_Coefficient"))
        print("D:\n", startup_log["pid_after_torque"].get("D_Coefficient"))
        print("Torque:\n", startup_log["pid_after_torque"].get("Torque_Enable"))
        print("PASS / FAIL\n", "PASS" if startup_log["pid_after_torque_pass"] else "FAIL")
        print("\n11. FIXED TARGET\n", fixed_start_enc)
        print("\n12. HOLD GOAL\n", q_hold_goal_raw)
        print("\n13. HOLD PRESENT\n", q_start_actual_raw)
        print("\n14. FRESH b_q\ndeg:\n", b_q_deg, "\nrad:\n", b_q_rad)
        print("\n15. ACTUAL CARTESIAN ANCHOR")
        print("X:", p0[0], "Y:", p0[1], "Z:", p0[2])
        print("Yaw:", euler_start[0], "Pitch:", euler_start[1], "Roll:", euler_start[2])
        print("\n16. FIRST COMMAND CONTINUITY")
        print("jump:\n", jump, f"\nmax:\n{np.max(np.abs(jump)):.3f}\nelbow:\n{jump[2]:.3f}")
        print("PASS / FAIL\n", "PASS" if np.max(np.abs(jump))<=0.5 else "FAIL")
        print("\n17. PREFLIGHT")
        print("IK:\n", pf_fails["IK"], "\nNaN:\n", pf_fails["NaN"], "\nhard:\n", pf_fails["hard"], "\nsafe:\n", pf_fails["safe"], "\nbranch:\n", pf_fails["branch"])
        print("PASS / FAIL\n", "PASS" if pf_ok else "FAIL")
        print("\n18. TRAJECTORY POWER CONTINUITY")
        print("power interruption:\n", "YES" if power_interruption else "NO")
        print("torque interruption:\n", "YES" if torque_interruption else "NO")
        print("bus disconnect:\n", "YES" if bus_disconnect else "NO")
        print("reconnect:\nNO")
        print("uninterrupted:\n", "NO" if (power_interruption or torque_interruption) else "YES")
        
        print("\n19. FULL LOG")
        print("ticks:\n", len(tick_logs["tick_index"]), "\nmissing:\n", steps - len(tick_logs["tick_index"]), "\nNaN:\n0")
        print("path:\n", log_dir / "step8_16r_full.npz")
        
        if len(tick_logs["tick_index"]) < steps or power_interruption or torque_interruption:
            print("\n35. STEP 8.16R DIAGNOSTIC VERDICT\nINVALID")
            return
        ...
        print("\n23. JOINT-TRACKING CARTESIAN RESIDUAL")
        ...
        print("\n28. FORMAL CARTESIAN GATES")
        print("positive Z:\n", "PASS" if dp_peak[2]>0 else "FAIL")
        print("peak 2.5–7.5mm:\n", "PASS" if (2.5 <= dp_peak[2]*1000 <= 7.5) else "FAIL")
        print("P95 <=5mm:\n", "PASS" if np.percentile(np.linalg.norm(e_total, axis=1), 95) <= 5.0 else "FAIL")
        print("return <=5mm:\n", "PASS" if ret_norm <= 5.0 else "FAIL")
        
        temps = slow_logs["temp"]
        print("\n29. TEMPERATURE")
        print(f"before:\n{temps[0]}")
        print(f"peak:\n{np.mean(temps[1:-1], axis=0) if len(temps)>2 else temps[-1]}")
        print(f"after:\n{temps[-1]}")
        print(f"max:\n{np.max(temps, axis=0)}")
        
        loads = slow_logs["load"]
        print("\n30. ELBOW LOAD RAW")
        print(f"before:\n{loads[0][2]}")
        print(f"peak:\n{np.mean(loads[1:-1], axis=0)[2] if len(loads)>2 else loads[-1][2]}")
        print(f"final:\n{loads[-1][2]}")
        ...
        print(f"\n34. LOW-LEVEL TRACKING LIMIT REPRODUCED?\n{reproduced}")
        
        valid = startup_log["goal_sync_pass"] and startup_log["pid_before_torque_pass"] and startup_log["pid_after_torque_pass"] and (startup_log['transient_max'][2] <= 2.0) and (np.max(np.abs(jump))<=0.5) and pf_ok and (not power_interruption) and (not torque_interruption)
