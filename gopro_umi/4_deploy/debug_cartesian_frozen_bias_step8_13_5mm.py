...
def run_preflight(ik_solver, traj_pos, T_start, q_actual_start_rad, b_q_rad):
    print("\nPreflight check for full Cartesian Trajectory...")
...
def main():
    ...
    try:
        ...
        print("difference vs STEP8.12 bias (deg):", b_q_deg - ref_b_q)
        ...
        print("\n--- Running +5mm CARTESIAN TRACKING ---")
        ...
        for step in range(steps):
            ...
            tick_timestamps.append(tick_start)
            ...
            compute_elapsed = time.perf_counter() - tick_start
        ...
        amp_err = abs(0.005 - dp_peak[2])*1000
        ratio = dp_peak[2] / 0.005
        print(f"Z amplitude error: {amp_err:.3f} mm")
        print(f"tracking ratio: {ratio*100:.3f} %")
        ...
        print(f"return norm: {ret_norm:.3f} mm")
        
        print("\nFORMAL TRACKING GATES:")
        print("positive Z: PASS" if dp_peak[2] > 0 else "positive Z: FAIL")
        print("peak range 2.5-7.5mm: PASS" if 2.5 <= dp_peak[2]*1000 <= 7.5 else "peak range 2.5-7.5mm: FAIL")
        print("P95 <= 5mm: PASS" if np.percentile(norms, 95) <= 5.0 else "P95 <= 5mm: FAIL")
        print("return <= 5mm: PASS" if ret_norm <= 5.0 else "return <= 5mm: FAIL")
        
        if (dp_peak[2] > 0 and 2.5 <= dp_peak[2]*1000 <= 7.5 and np.percentile(norms, 95) <= 5.0 and ret_norm <= 5.0):
            if amp_err <= 1.0 and ret_norm <= 2.0:
                print("QUALITY LABEL: GOOD")
            else:
                print("QUALITY LABEL: MARGINAL")
        else:
            print("QUALITY LABEL: FAIL")
        
        q_err_deg = np.rad2deg(np.array(logs["q_error"]))
        print("\nJOINT ERROR q_nom-q_actual (deg):")
        ...
        def get_rep(idx):
            res = {}
            res["q_nom"] = np.rad2deg(logs["q_nom"][idx])
            res["b_q"] = b_q_deg
            res["q_corr"] = logs["q_corr_applied"][idx]
            res["q_cmd"] = np.rad2deg(logs["q_cmd"][idx])
            res["q_actual"] = np.rad2deg(logs["q_actual"][idx])
            res["q_cmd-q_actual"] = res["q_cmd"] - res["q_actual"]
            return res
            
        def print_rep(name, data):
            print(f"\n{name}:")
            print("q_nom:", data["q_nom"])
            print("b_q:", data["b_q"])
            print("q_corr:", data["q_corr"])
            print("q_cmd:", data["q_cmd"])
            print("q_actual:", data["q_actual"])
            print("q_cmd-q_actual:", data["q_cmd-q_actual"])
        ...
        print_rep("START REPRESENTATIVE", rep_s)
        print_rep("PEAK REPRESENTATIVE", rep_p)
        print_rep("FINAL REPRESENTATIVE", rep_f)
        
        print("\nSUPPORT RESIDUAL CHANGE:")
        print("start:", rep_s["q_cmd-q_actual"])
        print("peak:", rep_p["q_cmd-q_actual"])
        print("final:", rep_f["q_cmd-q_actual"])
        print("peak-start:", rep_p["q_cmd-q_actual"] - rep_s["q_cmd-q_actual"])
        print("final-start:", rep_f["q_cmd-q_actual"] - rep_s["q_cmd-q_actual"])
        ...
        print("raw P95:", np.percentile(np.abs(q_corr_raw), 95, axis=0))
        print("applied max:", np.max(np.abs(q_corr_app), axis=0))
        print("clamp count:", clamp_count)
        print("percentage:", clamp_count/total_ticks*100)
        ...
        if "Temp" in logs:
            temps = np.array(logs["Temp"])
            print("peak:", np.mean(temps[idx_peak], axis=0))
            print("max:", np.max(temps, axis=0))
        ...
        c_time = np.array(compute_times) * 1000
        print("\nCOMPUTE WORK TIME (ms):")
        print(f"mean: {np.mean(c_time):.3f}")
        print(f"P95: {np.percentile(c_time, 95):.3f}")
        print(f"P99: {np.percentile(c_time, 99):.3f}")
        print(f"max: {np.max(c_time):.3f}")
        
        dt_tick = np.diff(tick_timestamps) * 1000
        print("\nACTUAL 30HZ TICK INTERVAL (ms):")
        print(f"mean: {np.mean(dt_tick):.3f}")
        print(f"STD: {np.std(dt_tick):.3f}")
        print(f"P95: {np.percentile(dt_tick, 95):.3f}")
        print(f"P99: {np.percentile(dt_tick, 99):.3f}")
        print(f"max: {np.max(dt_tick):.3f}")
        print(f"deadline overruns (>33.33ms): {np.sum(dt_tick > 33.34)}")
        ...
        logs_to_save = {
            ...
            "p_act": p_act,
            "euler_act": euler_act,
            "q_nom": np.array(logs["q_nom"]),
            "q_cmd": np.array(logs["q_cmd"]),
            "q_actual": np.array(logs["q_actual"]),
            "q_error": np.array(logs["q_error"]),
            "q_corr_raw": np.array(logs["q_corr_raw"]),
            "q_corr_applied": np.array(logs["q_corr_applied"]),
            "b_q_rad": b_q_rad,
            "b_q_deg": b_q_deg,
            "compute_times": np.array(compute_times),
            "tick_timestamps": np.array(tick_timestamps)
        }
        np.savez(DEPLOY_DIR / "step8_13_logs.npz", **logs_to_save)
        print("\nSaved logs to step8_13_logs.npz")
