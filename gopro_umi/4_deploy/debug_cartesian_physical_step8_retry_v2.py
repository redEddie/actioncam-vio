...
def move_to_start(follower, arm_names, q_start_enc_target, ik_solver, label="FIRST"):
    print(f"\n--- Moving to FIXED PHYSICAL START POSE ({label}) ---")
    obs = follower.bus.sync_read("Present_Position")
    ...
    # For safety limit check ONLY: convert to URDF radians
    ...
    for step in range(steps):
        ...
        alpha = (3.0 * progress**2) - (2.0 * progress**3) # smoothstep
    ...
    print("FIXED TARGET:")
    print(q_start_enc_target)
    print("ACTUAL AFTER HOLD:")
    print(mean_enc)
    print("ERROR TARGET - ACTUAL:")
    print(q_err)
    print(f"mean error: {np.mean(np.abs(q_err)):.3f}, max abs error: {np.max(np.abs(q_err)):.3f}")
    
    # Convert actual encoder to URDF for FK
    ...
    return q_actual_start_rad, pos_start, T_start[:3, :3], mean_enc
...
def run_cartesian_phase(follower, ik_solver, arm_names, anchor_q_rad, anchor_pos, anchor_rot, z_amp, phase_name):
    ...
    for i in range(0, num_pts, 2):
        ...
        # 10 degree jump check
        if np.max(np.abs(np.rad2deg(q_out - anchor_q_rad))) > 10.0:
            hard_violations += 1
            
        preflight_q_noms.append(q_out)
        q_seed = q_out
        
    if failures > 0 or safe_violations > 0 or nan_infs > 0 or hard_violations > 0:
        print(f"PREFLIGHT FAIL: IK fails={failures}, Safe viols={safe_violations}, Hard viols={hard_violations}, NaN/Inf={nan_infs}")
        return False
    ...
    try:
        for tick in range(int(dur * 30)):
            ...
            # Cartesian Emergency Gate (30mm)
            if np.linalg.norm(pos_act - anchor_pos) > 0.030:
                print("ABORT: 30mm emergency Cartesian gate exceeded!")
                return False
            
            # Joint Emergency Gate (10deg)
            if np.max(np.abs(np.rad2deg(q_act_rad - anchor_q_rad))) > 10.0:
                print("ABORT: 10deg emergency Joint gate exceeded!")
                return False
...
def main():
    ...
    # FIXED PHYSICAL START TARGET DEG
    q_start_target = np.array([-6.945, -81.462, 65.760, 46.132, 1.538], dtype=np.float64)
    print("START_POSE_SOURCE_FILE: Hardcoded from STEP 8-RETRY V2 instruction")
    print("START_POSE_REPRESENTATION: physical robot-side calibrated degree target (RAW ENCODER)")
    ...
    try:
        # 3. Move to fixed physical start pose
        q_anchor1, pos_anchor1, rot_anchor1, enc_anchor1 = move_to_start(follower, arm_names, q_start_target, ik_solver, label="FIRST")
        ...
        q_anchor2, pos_anchor2, rot_anchor2, enc_anchor2 = move_to_start(follower, arm_names, q_start_target, ik_solver, label="SECOND")
        
        # Repeatability check
        joint_diff = enc_anchor2 - enc_anchor1
