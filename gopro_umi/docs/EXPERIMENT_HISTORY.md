...
Historical sources and outputs are retained in the local canonical workspace
under `archive/` and are intentionally excluded from this source-only Git
upload; none of the values below override `CONTRACT.md`.
...
- The exact user-supplied `so101_gravity_compensation_v1.py` prototype was not
  ...
  `archive/patch_history/controller/gravity/so101_gravity_compensation_v1.py`.
...
The local-only archive taxonomy separates legacy deployment/training/dataset
code, patch history, controller/start experiments, old checkpoints, generated
results, upstream snapshots, and backups.
