...
- Each LeRobot row stores `observation.state: [6]` and `action: [6]` in
  ...
  the current sampled state within the same episode; the terminal row action
  is zero.
...
- Reference gravity bias degrees: `[0,0.615,2.286,0.703,0]`.
...
- Local immutable model bundle: `Delta_Weights/`.
