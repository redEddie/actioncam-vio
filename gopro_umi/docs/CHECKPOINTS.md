...
`Delta_Weights/` remains in place and immutable. Baseline and final hashes are:
...
The server-original files are kept byte-for-byte, including their original
server-root paths. The final launcher contract includes 20,000 steps, batch
...
are preserved only in the local canonical workspace archive and are excluded
from this source-only Git upload. They must not be used as the current
training contract.
...
- Source contract: the local, non-versioned yaw-free source Zarr
  `archive/legacy_dataset/zarr/replay_buffer_yawfree.zarr`, 10 Hz deterministic
  nearest-timestamp sampling from the audited `60000/1001 Hz` source clock,
  row-wise state/action shape `[6]`, step-to-step incremental action, zero
  terminal action, and no cross-episode delta.
- Local source validation covered all 77 episodes and 74,109 source frames;
  it produced 12,397 logical 10 Hz rows with zero duplicate selections, zero
...
server dataset
`/home/kimminje/gopro_umi/2_dataset/lerobot_dataset_10hz_chunk15_incremental_baseline_v1`.
...
### STEP 3A.1 real LeRobot validation (2026-08-13)

Using only
`/home/kimminje/miniconda3/envs/lerobot_env2/bin/python` and the pinned
project source at `third_party/lerobot` (HEAD
