...
void System::SaveTrajectoryAllMapsCSV(const string &filename)
{
    // Like SaveTrajectoryCSV, but exports frames of EVERY map in the atlas
    // with a map_id column.  Each map is a self-consistent trajectory in its
    // own (gravity-aligned, metric) frame anchored at its first keyframe.
    cout << endl << "Saving all-maps camera trajectory to " << filename << " ..." << endl;

    vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    std::map<Map*, Sophus::SE3f> map_Two;
    for(Map* pMap : vpMaps)
    {
        vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
        if(vpKFs.empty()) continue;
        sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
        map_Two[pMap] = vpKFs[0]->GetPoseInverse();
    }

    ofstream f;
    f.open(filename.c_str());
    f << fixed;
    f << "frame_idx,timestamp,state,is_lost,is_keyframe,map_id,x,y,z,q_x,q_y,q_z,q_w" << endl;
        ...
        iter_relative_pose++, iter_reference_keyframe++, iter_timestamp++, iter_is_lost++, iter_state++, frame_idx++)
    {
        ...
        KeyFrame* pKF = *iter_reference_keyframe;
        if (*iter_is_lost || !pKF){
            f << "true,false,-1,0,0,0,0,0,0,0" << endl;
            continue;
        }
        ...
        if(!pKF || map_Two.count(pKF->GetMap()) == 0)
        {
            f << "true,false,-1,0,0,0,0,0,0,0" << endl;
            continue;
        }
        ...
        Map* pMap = pKF->GetMap();
        bool is_keyframe = (pKF->mTimeStamp == *iter_timestamp);
        Trw = Trw * pKF->GetPose() * map_Two[pMap];
        Sophus::SE3f Twc = ((*iter_relative_pose) * Trw).inverse();
        Eigen::Vector3f twc = Twc.translation();
        Eigen::Quaternionf q = Twc.unit_quaternion();
    ...
    f.close();
    cout << "All-maps CSV camera trajectory saved!" << endl;
}
