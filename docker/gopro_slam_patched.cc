...
// Best-effort trajectory rescue on crash (ORB-SLAM3 has known races that
// can SIGSEGV mid-run; without this the whole trajectory is lost).
ORB_SLAM3::System* g_slam = nullptr;
std::string g_rescue_csv;
volatile sig_atomic_t g_in_crash = 0;
volatile sig_atomic_t g_ever_tracked = 0;  // SaveTrajectoryCSV needs >=1 KF
void crash_rescue_handler(int signum) {
   if (g_in_crash) _exit(signum);
   g_in_crash = 1;
   cout << "gopro_slam.cc FATAL signal " << signum
        << ", attempting trajectory rescue..." << endl;
   if (g_slam && g_ever_tracked && !g_rescue_csv.empty()) {
       g_slam->SaveTrajectoryCSV(g_rescue_csv);
       g_slam->SaveTrajectoryAllMapsCSV(g_rescue_csv + ".allmaps");
       cout << "rescued trajectory to " << g_rescue_csv << endl;
   }
   _exit(signum);
}
...
int main(int argc, char **argv) {
  ...
  // Crash-rescue: save trajectory on fatal signals once tracking has begun
  g_slam = &SLAM;
  if (!output_trajectory_csv.empty())
      g_rescue_csv = output_trajectory_csv + ".rescue";
  signal(SIGSEGV, crash_rescue_handler);
  signal(SIGABRT, crash_rescue_handler);
  signal(SIGBUS, crash_rescue_handler);
  ...
  for (int frame_idx=0; frame_idx < nImages; frame_idx++){
    ...
    while(last_imu_idx < imuTimestamps.size() &&
          imuTimestamps[last_imu_idx] <= tframe && tframe > 0)
    {
    ...
    // periodic checkpoint so a late crash doesn't lose the whole trajectory
    if (g_ever_tracked && !output_trajectory_csv.empty()
        && frame_idx % 1500 == 1499) {
        SLAM.SaveTrajectoryCSV(output_trajectory_csv + ".ckpt");
    }
  ...
  // Save camera trajectory BEFORE Shutdown: Shutdown() has known thread
  // races that can crash and would lose the outputs
