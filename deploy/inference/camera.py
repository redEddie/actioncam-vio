...
def prepare_transport_rgb(
    bgr: np.ndarray,
    size: tuple[int, int] = (256, 256),
    *,
    remove_pillarbox: bool = True,
    black_threshold: float = 8.0,
) -> np.ndarray:
    """Preprocess camera frame to match the full-FOV square-squeeze training contract.

    Training contract:
        GoPro 8:7 original (3840x3360) -> NO SCENE CROP -> cv2.resize((224, 224)) -> (256, 256).
    Live contract:
        - Case A (e.g. 1232x1080 ~ 8:7 native / Full FOV):
          NO SCENE CROP -> direct square squeeze resize.
        - Case B (1920x1080 with black pillarbox):
          Strip left/right black padding -> 8:7 active image region -> square squeeze resize.
        - Case C (1920x1080 already scene-cropped):
          Full frame squeeze resize (never center crop scene).
    """
    ...
    if remove_pillarbox:
        # Check column-wise mean brightness to detect black padding
    ...
    # Vertical flip to reconcile live HDMI orientation with training dataset geometry
    rgb = cv2.flip(rgb, 0)

    # Full FOV square squeeze matching training build_umi_zarr.py (NO CENTER CROP)
