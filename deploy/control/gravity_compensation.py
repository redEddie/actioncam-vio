...
class GravityCompensator:
    """Cached URDF gravity model plus reference-scaled signed compensation."""
    ...
    def __init__(
        ...
        *,
        ...
        reference_commands = self.gravity_reference_bias_deg[list(self.pitch_indices)]
        reference_torques = self.tau_ref_nm[list(self.pitch_indices)]
        if np.any(reference_commands * reference_torques >= 0.0):
            raise GravityModelError(
                "reference gravity compensation must oppose gravity torque on every pitch joint"
            )
    ...
    def gravity_torque(
        ...
        *,
        ...
        """Return torque exerted by gravity, ``tau_g = -dU/dq``, in Nm.

        This is the generalized load on each joint, not the actuator torque
        needed to counter gravity. Payload flags support explicit ablations.
        """
    ...
    def evaluate(self, q_actual_urdf_rad: Sequence[float]) -> GravityEvaluation:
        ...
        # The ratio transports load magnitude/sign away from the reference
        # pose. The configured signed command supplies the opposing direction.
