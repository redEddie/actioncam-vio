...
def test_reference_anchor_exact_and_denominator_guard(config, gravity):
    ...
    expected_bias = np.array([0.264, -0.615, -2.286, -0.703, -0.352])
    assert np.array_equal(evaluation.gravity_bias_deg, config.gravity_reference_bias_deg)
    assert np.array_equal(evaluation.total_support_bias_deg, expected_bias)
    ...
    assert np.all(
        config.gravity_reference_bias_deg[[1, 2, 3]] * gravity.tau_ref_nm[[1, 2, 3]] < 0.0
    )
    with pytest.raises(GravityModelError, match="near-zero reference"):
        _build(config, denominator_guard_nm=100.0)
    with pytest.raises(GravityModelError, match="must oppose gravity torque"):
        _build(config, gravity_reference_bias_deg=-config.gravity_reference_bias_deg)
